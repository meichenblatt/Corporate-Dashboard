...
var DashboardApp=angular.module("DashboardApp",["ngRoute","ngResource","uiGmapgoogle-maps","chart.js"]);DashboardApp.config(function($routeProvider,$locationProvider){$routeProvider.when("/",{controller:"geospatialController",templateUrl:"tmpl/geospatial.html"}).when("/keymetrics",{controller:"keymetricsController",templateUrl:"tmpl/keymetrics.html"}).when("/dataview",{controller:"dataviewController",templateUrl:"tmpl/dataview.html"}).otherwise({redirect:"/"}),$locationProvider.html5Mode(!0)}),DashboardApp.factory("Locations",function($resource){return $resource("https://udacity-5c1b2.firebaseio.com/Locations.json")}),DashboardApp.factory("ReportedIssues",function($resource){return $resource("https://udacity-5c1b2.firebaseio.com/KeyMetrics/Reported%20Issues.json")}),DashboardApp.factory("Customers",function($resource){return $resource("https://udacity-5c1b2.firebaseio.com/KeyMetrics/Customers.json")}),DashboardApp.factory("OpenIssues",function($resource){return $resource("https://udacity-5c1b2.firebaseio.com/KeyMetrics/OpenIssues.json")});
...
DashboardApp.controller("keymetricsController",function($scope,ReportedIssues,Customers,OpenIssues,$timeout){function getMetrics(){OpenIssues.get({},function(openIssues){$scope.openIssues=openIssues.OpenIssues}),ReportedIssues.query(function(reports){$scope.labels=[],$scope.data=[];var data=[];reports.forEach(function(report){$scope.labels.push(report.dates),data.push(report.data)}),$scope.data.push(data)}),Customers.query(function(customers){$scope.Cuslabels=[],$scope.Cusdata=[];var data=[];customers.forEach(function(customer){$scope.Cuslabels.push(customer.dates),data.push(customer.data)}),$scope.Cusdata.push(data)})}$scope.series1=["Open Issues"],$scope.series2=["Paying Customers"],getMetrics(),$timeout(getMetrics,3e4),$scope.options={scales:{yAxes:[{id:"y-axis-1",type:"linear",display:!0,position:"left"}]}}});
DashboardApp.controller("dataviewController",function($scope,$http,$timeout){function getIssues(){$http.get("/data/issues.csv").success(function(issues){var lines=issues.split("\n");$scope.Issues=[],lines.forEach(function(line,indx){if(0===indx);else{var temp=line.split(","),obj={};obj.id=parseInt(temp[0]),obj.created=new Date(temp[1]),obj.closed=new Date(temp[2]),obj.name=temp[3],obj.email=temp[4],obj.description=temp[5],$scope.Issues.push(obj)}})})}$scope.sortType="id",$scope.sortReverse=!1,$scope.search="",getIssues(),$timeout(getIssues,3e4)});